...
haven't violated any of the requirements in this handout. For example, we will manually validate that there is no class-specific logic in the `insertRow` and `loadRow` methods, and if there is, we will deduct points manually. The Gradescope assignment will be up by 1/26.
